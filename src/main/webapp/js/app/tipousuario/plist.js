...
moduleTipousuario.controller('tipousuarioPlistController', ['$scope', '$http', '$location', 'toolService',
    ...
    function ($scope, $http, $location, toolService, oSessionService, $routeParams) {
        ...
        $http({
            ...
            url: 'http://localhost:8081/tailorShop/json?ob=tipousuario&op=getpage&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/tailorShop/json?ob=tipousuario&op=getcount'
        }).then(function (response) {
            $scope.status = response.status;
