...
moduleUsuario.controller('usuarioPlistController', ['$scope', '$http', '$location', 'toolService',
    ...
    function ($scope, $http, $location, toolService, $routeParams, oSessionService) {
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/tailorShop/json?ob=usuario&op=getpage&rpp=' + $scope.rpp + '&page=' + $scope.page + $scope.orderURLServidor
        ...
        $http({
            method: 'GET',
            url: 'http://localhost:8081/tailorShop/json?ob=usuario&op=getcount'
        }).then(function (response) {
            $scope.status = response.status;
        ...
        $scope.crearUsuarios = function () {
            $http({
                method: 'GET',
                url: 'http://localhost:8081/tailorShop/json?ob=usuario&op=fill'
            }).then(function (response) {
                $scope.status = response.status;
